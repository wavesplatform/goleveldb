	...
	for i := range n {
		...
		go func(i int) {
			...
			for k := range niter {
				batch.Reset()
				for j := range bk {
					batch.Put(fmt.Appendf(nil, "batch-%d.%d.%d", i, k, j), fmt.Appendf(nil, "v%d", k))
				}
				h.write(batch)
				// Key should immediately available after put returns.
				for j := range bk {
	...
	go func() {
		...
		for ; time.Now().Before(until) && atomic.LoadUint32(&done) == 0; i++ {
			...
			for k := range n {
				...
				b.Put(fmt.Appendf(nil, "PTR%06d", k), []byte(key))
			}
			h.write(b)
...
func TestDB_TableCompactionBuilder(t *testing.T) {
	...
	for i := range 2 {
		...
		for k := 0; tw.tw.BytesLen() < targetSize; k++ {
			key := fmt.Appendf(nil, "%09d", k)
			seq += nSeq - 1
			for x := range uint64(nSeq) {
